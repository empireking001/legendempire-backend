...
exports.createCategory = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ success: true, data: cat, message: "Category created." });
  } catch (e) {
    if (e.code === 11000)
      return res
        .status(400)
  ...
  try {
    ...
    if (count > 0)
      return res
        ...
        .json({
          success: false,
          message: `Cannot delete — ${count} post(s) use this category.`,
        });
  ...
  try {
    ...
    for (const cat of categories) {
      const posts = await Post.find({ status: "published", category: cat._id })
        ...
        .select("title slug excerpt coverImage publishedAt views readTime")
        .sort({ publishedAt: -1 })
        .limit(6);
...
exports.getTrending = async (req, res) => {
  try {
    const posts = await Post.find({ status: "published" })
      ...
      .select("title slug coverImage views publishedAt readTime")
      .sort({ views: -1 })
      .limit(6);
...
exports.likePost = async (req, res) => {
  try {
    ...
    if (liked) {
      post.likedBy = post.likedBy.filter((a) => a !== ip);
...
exports.createPost = async (req, res) => {
  try {
    ...
    // ✅ Cleaned up out-of-scope crashing bug. Telegram fires safely inside try block.
    if (post.status === "published") {
      try {
        const { autoPostToTelegram } = require("./telegramController");
        autoPostToTelegram(populated).catch((tgErr) =>
          console.error("Telegram post failed:", tgErr.message),
        );
      } catch (requireErr) {
        console.error("Could not load telegramController:", requireErr.message);
      }
    }
  } catch (err) {
...
exports.publishPost = async (req, res) => {
  try {
    ...
    // Increment category postCount when draft transitions to live
    ...
    // Respond immediately to avoid frontend timeout or spinner lockup
    res.json({ success: true, message: "Post published!", data: populated });

    // Handle non-blocking heavy processes asynchronously in background threads
    process.nextTick(async () => {
      try {
        const { autoPostToTelegram } = require("./telegramController");
        await autoPostToTelegram(populated);
      } catch (err) {
...
exports.addAdmin = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: `${name} added as ${role || "admin"}.`,
        data: {
          id: user._id,
          name: user.name,
          email: user.email,
